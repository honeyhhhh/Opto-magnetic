...
/* RESERVED										0x00800000 */
/* RESERVED										0x01000000 */
#define	OPTOTRAK_BACKGROUND_SUBTRACT_ON		0x00200000
#define	OPTOTRAK_BACKGROUND_SUBTRACT_OFF		0x00000000
/* RESERVED										0x04000000 */
/* RESERVED										0x08000000 */
/* RESERVED										0x10000000 */
/* RESERVED										0x20000000 */
...
#define CENTROID_HAS_SIDE_PEAK         13
#define CENTROID_MISSING_FRAME         14
#endif
...
#define MAXIMUM_NUMBER_OF_SROM_PORTS    6
#define SROM_SERIAL_NUMBER_SIZE			8
...
#define DH_DEVICE_TYPE_OPTOTRAK_PRO_PROBE_CONTROLLER	0x02
...
#define DH_DEVICE_TYPE_TOUCH_PROBE_MSP_CONTROLLER		0x08
#define DH_DEVICE_TYPE_WIRELESS_DEVICE_CONTROLLER		0x09
#define DH_DEVICE_TYPE_UNIVERSAL_CONTROLLER				0x0A
...
enum DeviceHandleProperties
{
	...
	DH_PROPERTY_SELECT_PROBE_CLUSTER = 48,
	...
	DH_PROPERTY_SROM_IS_CACHED = 55,
	DH_PROPERTY_SROM_ID = 56
}; /* enum DeviceHandleProperties */
...
{
	int nID;
	enum DeviceHandleStatus dtStatus;
} DeviceHandle;
...
Description:
	...
	uPropertyID		unsigned integer identifying the property stored in the structure.
					possible properties:
						...
						DH_PROPERTY_SROM_IS_CACHED		indicates if the device's SROM is cached on the SCU
						DH_PROPERTY_SROM_ID SROM Serial numbers of all attached tools
...
typedef struct
{
	...
	enum DeviceHandlePropertyType dtPropertyType;
	union
	{
		int nData;
		float fData;
		...
		long lData;
		char cData;
		char szData[DEVICE_MAX_PROPERTY_STRING];
	} dtData;
...
typedef struct
{
	DeviceHandle *pdtHandle;
	DeviceHandleProperty *grProperties;
	int nProperties;
	boolean bRigidBody;
} DeviceHandleInfo;
...
//----------------------------------------------------------------------------------------------
// OAPI Extended Error Codes (OEE_*)   returned from OptoGetExtendedErrorCode()
...
//----------------------------------------------------------------------------------------------
// table to provide string version of error codes, provided through OptoLookupErrorCode()
...
#define OPTO_MAX_MARKERFREQ_CERTUS			( 4608.3f + FLOAT_COMP_EPS )	// 4600 converted to the nearest 1 MHz tick
#define OPTO_MIN_FRAMEFREQ					OPTO_MIN_MARKERFREQ
#define OPTO_MAX_FRAMEFREQ					( OPTO_MAX_MARKERFREQ / ( OPTO_MIN_MARKERS + 1 ) )
#define OPTO_MAX_FRAMEFREQ_CERTUS			( OPTO_MAX_MARKERFREQ_CERTUS / ( OPTO_MIN_MARKERS + 1.0f ) + FLOAT_COMP_EPS )
...
NDI_DECL1 int   NDI_DECL2 OptoGetLastError( char* buf, int len );
NDI_DECL1 int 	NDI_DECL2 OptoGetExtendedErrorCode();
...
NDI_DECL1 int   NDI_DECL2 OptotrakGetApiRev( int nNodeId, char *pszApiRev );
...
NDI_DECL1 void NDI_DECL2 OptoCombineXfrms( transformation *xfrm1_ptr, transformation *xfrm2_ptr, transformation *NewXfrmPtr );
NDI_DECL1 void NDI_DECL2 OptoCvtQuatToRotationMatrix( struct QuatRotationStruct *QuatPtr, RotationMatrixType r );
NDI_DECL1 void NDI_DECL2 OptoCvtRotationMatrixToQuat( struct QuatRotationStruct *QuatPtr, RotationMatrixType r );
NDI_DECL1 void NDI_DECL2 OptoDetermineEuler( RotationMatrixType r, rotation *RotationAnglePtr );
NDI_DECL1 void NDI_DECL2 OptoDetermineR( rotation *RotationAnglePtr, float ( *RotationMatrix )[ 3] );
NDI_DECL1 void NDI_DECL2 OptoInverseXfrm( transformation *InputXfrmPtr, transformation *OutputXfrmPtr );
NDI_DECL1 void NDI_DECL2 OptoTransformPoint(	float ( *RotationMatrix )[ 3], Position3d *translation, Position3d *OriginalPositionPtr,
												Position3d *RotatedPositionPtr );
NDI_DECL1 int NDI_DECL2 OptoFileOpen(	char* pszFilename, UINT uFileId, UINT uFileMode, int* pnItems, int* pnSubItems, long int *plnFrames,
										float* pfFrequency, char* pszComments, void** pFileHeader );
NDI_DECL1 int NDI_DECL2 OptoFileRead( UINT uFileId, long int lnStartFrame, UINT uNumberOfFrames, void *pDataDest );
NDI_DECL1 int NDI_DECL2 OptoFileWrite( UINT uFileId, long int lnStartFrame, UINT uNumberOfFrames, void *pDataSrc );
NDI_DECL1 int NDI_DECL2 OptoFileClose( UINT uFileId );
NDI_DECL1 int NDI_DECL2 OptoFileOpenAll(	char *pszFilename, UINT uFileId, UINT uFileMode, int *pnItems, int *pnSubItems, int *pnCharSubItems,
											int *pnIntSubItems, int *pnDoubleSubItems, long int *plnFrames, float *pfFrequency, char *pszComments,
											void **pFileHeader );
NDI_DECL1 int NDI_DECL2 OptoFileReadAll(	UINT uFileId, long int lnStartFrame, UINT uNumberOfFrames, void *pDataDestFloat, void *pDataDestChar,
											void *pDataDestInt, void *pDataDestDouble );
NDI_DECL1 int NDI_DECL2 OptoFileWriteAll(	UINT uFileId, long int lnStartFrame, UINT uNumberOfFrames, void *pDataSrcFloat, void *pDataSrcChar,
											void *pDataSrcInt, void *pDataSrcDouble );
NDI_DECL1 int NDI_DECL2 OptoFileCloseAll( UINT uFileId );
