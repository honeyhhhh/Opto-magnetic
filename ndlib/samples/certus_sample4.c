...
void main( int argc, char *argv[] )
{
	...
	for( nCurDevice = 0; nCurDevice < nDevices; nCurDevice++ )
	{
		...
		for( nCurProperty = 0; nCurProperty < pdtDeviceHandlesInfo[nCurDevice].nProperties; nCurProperty++ )
		{
			...
			case DH_PROPERTY_SROM_ID:
				sprintf ( szProperty, " SROM ID number ");
				break;
			...
			switch( pdtDeviceHandlesInfo[nCurDevice].grProperties[nCurProperty].dtPropertyType )
			{
				...
				case DH_PROPERTY_TYPE_STRING:
					...
					case DH_PROPERTY_SROM_ID:
						pChar = pdtDeviceHandlesInfo[nCurDevice].grProperties[nCurProperty].dtData.szData;
						{
							unsigned uPort;
							int i;

							pChar--;      /* Predecrement */
							for ( uPort = 0; uPort < MAXIMUM_NUMBER_OF_SROM_PORTS; uPort++ )
							{
								fprintf ( stdout, "    SROM %d:", uPort );
								for ( i = 0; i < SROM_SERIAL_NUMBER_SIZE; i++ )
								{
									fprintf ( stdout, "%02x", *++pChar );
								}
								fputc( '\n', stdout );
							}
						} 
