...
Description:
	...
	- Activate the markers.
	- Request/receive/display 3 frames of 3D data.
	- De-activate the markers.
...
void main( int argc, char *argv[] )
{
	...
	fprintf( stdout, "\n3D Data in original coordinate sytem\n" );
	for( uMarkerCnt = 0; uMarkerCnt < NUM_MARKERS; ++uMarkerCnt )
    {
     ...
     */
    for( uMarkerCnt = 0; uMarkerCnt < NUM_MARKERS && uFramesToAvg > 0; uMarkerCnt++ )
    {
	...
	/* calculate desired positions as average positions relative to the first marker */
	for( uMarkerCnt = 0; uMarkerCnt < NUM_MARKERS; uMarkerCnt++ )
    {
		grdtAlignedPositions[uMarkerCnt].x = grdtAveraged3dData[uMarkerCnt].x - grdtAveraged3dData[0].x;
		grdtAlignedPositions[uMarkerCnt].y = grdtAveraged3dData[uMarkerCnt].y - grdtAveraged3dData[0].y;
		grdtAlignedPositions[uMarkerCnt].z = grdtAveraged3dData[uMarkerCnt].z - grdtAveraged3dData[0].z;
	}
    ...
    if( OptotrakChangeCameraFOR(
            ...
            grdtAlignedPositions,	/* Marker positions in desired FOR. */			
            grdtAveraged3dData,		/* Marker positions in current FOR. */
     ...
     */
    fprintf( stdout, "\n\n3D Data in NEW coordinate system\n" );
    for( uFrameCnt = 0; uFrameCnt < 3; ++uFrameCnt )
    {
        /*
         * Get a frame of data.
         */
